...
Route::get('/test-brevo', function () {
    \Log::info("BREVO_TEST: Starting direct SMTP test...");
    
    try {
        $startTime = microtime(true);
        
        \Illuminate\Support\Facades\Mail::raw('Brevo SMTP direct test from FarmGrid!', function ($message) {
            $message->to('manveersingh0501l@gmail.com')
                    ->subject('Brevo SMTP Direct Test');
        });

        $duration = round(microtime(true) - $startTime, 2);
        
        \Log::info("BREVO_TEST: Success!", ['duration' => $duration]);
        return "Brevo Mail Sent Successfully in {$duration}s!";
        
    } catch (\Symfony\Component\Mailer\Exception\TransportExceptionInterface $e) {
        $duration = round(microtime(true) - $startTime, 2);
        \Log::error("BREVO_TEST: Transport Failure", [
            'message' => $e->getMessage(),
            'duration' => $duration,
            'host' => config('mail.mailers.smtp.host'),
            'port' => config('mail.mailers.smtp.port')
        ]);
        return "Brevo Mail Failed (Transport): " . $e->getMessage() . " after {$duration}s";
        
    } catch (\Exception $e) {
        \Log::error("BREVO_TEST: General Failure", ['message' => $e->getMessage()]);
        return "Brevo Mail Failed: " . $e->getMessage();
    }
});
